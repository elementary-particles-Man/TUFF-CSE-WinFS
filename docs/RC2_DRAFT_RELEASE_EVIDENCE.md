...
Because GitHub's per-run `GITHUB_TOKEN` cannot read draft releases, the verification step uses the `P7G_DRAFT_READ_FINE_GRAINED_TOKEN` Actions secret. The verifier uses the credential only for repository and Actions reads, and it is never written to evidence or logs. P7H moves that secret to a fine-grained PAT with repository access restricted to TUFF-CSE-WinFS and only `contents: read` and `actions: read`.
