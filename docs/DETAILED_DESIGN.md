...
- **Verification Order:** `rc-status` → manifest verify → checksum verify → draft release input verify → draft release create の順で確認する。
- **Workflow Boundary:** `draft-github-release.yml` は `workflow_dispatch` のみで起動し、publish は行わない。
...
- `release/verify-draft-release-inputs.ps1` validates the asset set and checksum alignment before release creation.
- `release/create-draft-github-release.ps1` creates a draft prerelease only and fails closed if the tag or release already exists.
