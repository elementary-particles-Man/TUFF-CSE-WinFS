...
### 9.11 P7C: RC Tag / Draft GitHub Release Asset Boundary
P7Cは、固定した `v1.0.0-rcN` タグ候補を draft GitHub Release の asset boundary に接続するフェーズである。ここでは publish、live install、driver signing、CSE crypto I/O、TPM live API、live KMS/HSM/CloudKMS/PKCS#11 接続を追加しない。
P7Eは、その draft release boundary を維持したまま、workflow ref と release target commit を分離して再現可能にする。
...
- **Verification Order:** `rc-status` → manifest verify → checksum verify → draft release input verify → `validate_only=true` の bundle verify → draft release create の順で確認する。release target は workflow checkout HEAD ではなく既存 RC tag の target commit と照合する。
...
### 12.8 P7C RC Tag / Draft GitHub Release Asset Boundary
P7C connects the fixed RC tag candidate to a draft GitHub Release asset boundary without publishing a release.

- `release/RC_TAG_POLICY.md` fixes the RC tag format to `v1.0.0-rcN`.
- `release/V1_RC_DRAFT_RELEASE_INPUT.template.json` defines the draft release input boundary and separates `workflow_ref` from `release_target_commitish`.
- `release/verify-draft-release-inputs.ps1` validates the asset set, workflow ref, and checksum alignment before release creation.
- `release/create-draft-github-release.ps1` creates a draft prerelease only, supports `-ValidateOnly`, and fails closed if the tag or release already exists.
- `.github/workflows/draft-github-release.yml` runs only on `workflow_dispatch`.
- P7E keeps the existing draft release untouched while making the workflow reproducible from the workflow ref rather than the release target commit.
- P7C attaches only the public installer zip, manifest, checksum report, and draft release notes.
- P7C keeps GitHub Release publish, live driver install, `pnputil`, signing, service install, live KMS/HSM/CloudKMS/PKCS#11, TPM live API, and CSE crypto I/O out of scope.
