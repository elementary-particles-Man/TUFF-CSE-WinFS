...
- **Verification Order:** `rc-status` → manifest verify → checksum verify → draft release input verify → `validate_only=true` の bundle verify → draft release create の順で確認する。
