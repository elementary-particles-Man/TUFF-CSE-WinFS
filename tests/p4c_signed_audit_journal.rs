...
mod tests {
    ...
    use tuff_cse_winfs::audit_signing::{DevAuditSigner, AuditSigner};
    ...
    fn test_tamper_detection_on_record_hash() {
        ...
        store.save_audit_public_key(&signer.public_key_record()).unwrap();
        ...
        operation_journal::append_signed_record(store.root_path(), &vol_hash, record, &[0u8; 32], &signer).unwrap();

        // 2. Tamper with record_hash
        let mut records = operation_journal::read_journal_records(store.root_path(), &vol_hash).unwrap();
